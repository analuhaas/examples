...
/**
 * @brief  This example deploys the open-loop control of a MMC arm without Capacitor Voltage Balancing algorithm. 
...
void loop_critical_task()
{
    ...
    if (mode == POWERMODE)
    {
        if (module_ID == MMC_LEAD) //CONTROL INSIDE LEAD - Modulation NLM + sets modules connection order
        {
            ...
            /* Modules state assignment according to preference order */
            for (uint8_t counter = 0; counter < total_number_of_modules_arm; counter++) {
